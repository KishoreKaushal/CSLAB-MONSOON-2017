...
int checkEdgeInWeightedPath(int *shortestPath , int vertex1 , int vertex2 ){
    for (size_t i=1; shortestPath[i]!=-1 ; i++ ){
        if(shortestPath[i-1]==vertex1 && shortestPath[i]==vertex2)
        return 1;
    }
    for (size_t i=1; shortestPath[i]!=-1 ; i++){
...
void publishShortestPathInWeightedGraph( _GRAPH_ * graph ,int *shortestPath){
    char outFileName[30];
    printf("\nEnter the output \".dot\" file name (without any spaces) : ");
    scanf(" %s" , outFileName);
    FILE *fout = fopen(outFileName, "w+");
    ...
    for (int i=0; i<graph->vc ; i++){
        ...
        for (int j=i; j<graph->vc ; j++){
            if(graph->AM[i][j]>=1){
                if(checkEdgeInWeightedPath(shortestPath, i ,j))
                fprintf(fout, "%d -- %d  [label=%d] [color=red];\n", i , j , graph->AM[i][j]);
                else fprintf(fout, "%d -- %d [label=%d] ;\n", i , j,graph->AM[i][j]);
                isolation = 0;
            }
        }
        // IF A POINT IS ISOLATED
...
int * dijkstra(_GRAPH_ *graph , _GNODE_ **root , _GNODE_ **goal){
    int *visited = (int *)malloc(sizeof(int) * graph->vc);
    int *shortestDistFromRoot = (int *)malloc(sizeof(int)*graph->vc);
    int *prevVertex = (int *)malloc(sizeof(int)*graph->vc);

    // initialization of the array
    for(int i=0; i<graph->vc; i++ ){
        visited[i] = 0;
        shortestDistFromRoot[i] = INT_MAX;
        prevVertex[i] = -1;
    }

    int x , y;
    x = (*root)->nodeNumber;        // start node number
    y = (*goal)->nodeNumber;        // end node number
    shortestDistFromRoot[x] = 0;
    int curr , adjNodeDist , currNodeDist, adjVertexNum;
    _GNODE_ **currentNode;

    // main content

    for(int i=0; i<graph->vc; i++){
        // select the unvisited vertex with shortest distance from root node
        curr = -1;
        for(int j=0; j<graph->vc ; j++){
            if(visited[j] == 0 && curr == -1) {
                curr = j ;
            } else if(visited[j]==0 && (shortestDistFromRoot[curr] > shortestDistFromRoot[j])){
                curr = j;
            }
        }
        if(curr == -1) break;
        currentNode = &graph->node[curr];
        currNodeDist = shortestDistFromRoot[curr];
        visited[curr] = 1;

        // for all adjacent nodes of the current node
        for(int k=0; k < (*currentNode)->adjNum; k++){
            adjVertexNum = (*(*currentNode)->adjacent[k])->nodeNumber;
            adjNodeDist = shortestDistFromRoot[adjVertexNum];
            //printf("Loop i: %d k: %d \t adjNodeDist : %d \t adjVertexNum: %d\n",i, k, adjNodeDist, adjVertexNum);
            if( adjNodeDist > (currNodeDist + graph->AM[adjVertexNum][curr] ) ){
                shortestDistFromRoot[adjVertexNum]  =  (currNodeDist + graph->AM[adjVertexNum][curr] ) ;
                prevVertex[adjVertexNum] = curr;
            }
        }
    }
    printf("\nNodes\tVisited\t\tShortest-Dist-From-Root\t\tPrev-Vertex\n");
    for (int i=0; i < graph->vc ; i++ ){
        printf("%d\t%d\t\t%d\t\t\t\t%d\n", i , visited[i] , shortestDistFromRoot[i] , prevVertex[i]);
    }
    printf("Shortest Path Length from node %d to %d : %d\n", x , y , shortestDistFromRoot[y]);

    // backtracing the path
    if(shortestDistFromRoot[y]==0){
        free(visited);
        free(shortestDistFromRoot);
        free(prevVertex);
        return NULL;
    }
    ...
    path[0] = y;
    while(prevVertex[bktrace]!=-1) {
        bktrace = path[cnt-1] = prevVertex[bktrace];
        cnt++;
        path = (int *)realloc(path , sizeof(int )*(cnt+1));
    }
    path[cnt-1] = x;
    path[cnt] = -1;
    free(visited);
...
void makeDFSTreeGraph( _GRAPH_ * graph ,int *prev){
    char outFileName[30];
    printf("\nEnter the output \".dot\" file name (without any spaces) : ");
    scanf(" %s" , outFileName);
    FILE *fout = fopen(outFileName, "w+");
...
int DFSpublishTreeByIterativeMethod(_GRAPH_ *graph,     _GNODE_ **root){
    int *visited = (int *)malloc (sizeof(int) *graph->vc);
    int *order = (int *)malloc(sizeof(int) *graph->vc);
    int *prev = (int *)malloc(sizeof(int) * graph->vc);
    ...
    int orderCounter=0 , flag=0, curr, w, prevTop = (*root)->nodeNumber;

    _STACK_ s;  initializeStack(&s);
    push(&s, root);

    //printf("Start Node: %d , Addr: %d\n", (*root)->nodeNumber , root );
    //printf("End Node: %d , Addr: %d\n", (*goal)->nodeNumber , goal );
    while (!stackEmpty(&s)){
        _GNODE_ **current = (_GNODE_ *) pop(&s);
        curr = (*current)->nodeNumber;

        if(!visited[curr]) {
            printf("\n %d", curr);
            order[orderCounter++] = curr;
            visited[curr] = 1;
            for (int i=0; i<(*current)->adjNum; i++){
                    // w = (*(*current)->adjacent[i])->nodeNumber;
                    // prev[1][w] = curr;
                    push(&s, (*current)->adjacent[i]);
            }
        }
    }

    // create  prev matrix from order matrix
    prev[(*root)->nodeNumber] = (*root)->nodeNumber;
    for (int i=1; i<graph->vc; i++ ){
        // for current node in the order find the most recent adjacent node in the order
        for (int j=i-1 ; j>=0 ; j--) {
            if(graph->AM[order[j]][order[i]] >=1){
                prev[order[i]] = order[j];
                break;
            }
        }
    }

    makeDFSTreeGraph(graph, prev);
